...
def tosun_convert_msg(msg):
    """
    TLIBCAN  TLIBCANFD msg convert to app_can.Message
    Easy python-app_can to use
    """
...
def msg_convert_tosun(msg):
    """
    app_can.Message convert to  TLIBCAN  TLIBCANFD msg
    Easy python-app_can to use
    """
    if isinstance(msg, TLIBCAN):
        return msg
    ...
    def __Creat_Msg(self, _msg: int or str, AChannel=0):
        if isinstance(_msg, int):
            if _msg in self.dbc_list_by_id:
                ...
                for i in range(Fdlc):
                    DataList.append(0)
                return Message(channel=AChannel, arbitration_id=_msg, is_fd=self.dbc_list_by_id[_msg]._is_fd,
                               is_extended_id=self.dbc_list_by_id[_msg]._is_extended_frame, dlc=Fdlc, data=DataList)
            ...
            if _msg in self.dbc_list_by_name:
                ...
                for i in range(Fdlc):
                    DataList.append(0)
                return Message(channel=AChannel, arbitration_id=self.dbc_list_by_name[_msg]._frame_id,
                               is_fd=self.dbc_list_by_name[_msg]._is_fd,
                               is_extended_id=self.dbc_list_by_name[_msg]._is_extended_frame, dlc=Fdlc, data=DataList)
        return None
