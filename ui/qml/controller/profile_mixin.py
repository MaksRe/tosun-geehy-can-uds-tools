...
#
# 1 - серия из пяти измерений;
# 2 - серия из 11 измерений, ровно период сети 50 Гц (прошивка с b0081).
# Прибор со старой прошивкой ждёт 1 и профиль с номером 2 не применит: прошивку
# и программу настройки нужно обновлять вместе.
PROFILE_ALGORITHM_ID = 2
...
class AppControllerProfileMixin:
    ...
    def _profile_device_status_text(self) -> str:
        ...
        if not (status & 0x01):
            return "В приборе профиль пустой: таблицы нулевые, поправки по температуре нет"
