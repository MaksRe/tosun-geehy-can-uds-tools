...
class AppControllerDiagnosticsMixin(AppControllerContract):
    ...
    # Пороги дрожания измерения (размаха серии), отсчёты. 33 отсчёта это примерно 0,8 пФ.
    #
    # Пороги рассчитаны на серию из 11 выборок (прошивка с b0081). Размах - это
    # разница между крайними выборками серии, и у большей серии крайние значения
    # при том же шуме дальше друг от друга: у 11 выборок в 1,36 раза, чем у 5.
    # Прежние пороги 12/24/48 были для серии из 5 и помножены на 1,36, чтобы
    # один и тот же контур получал ту же оценку, что и раньше.
    DIAGNOSTICS_SPREAD_GOOD = 16
    DIAGNOSTICS_SPREAD_WARN = 33
    DIAGNOSTICS_SPREAD_BAD = 65
